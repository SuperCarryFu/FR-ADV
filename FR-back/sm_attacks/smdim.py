...
class MetaDIM(ConstrainedMethod):
    ...
    def batch_grad(self,x, img_ft, alpha,model):
        global_grad = 0
        for _ in range(12):
            x_neighbor1 = x + torch.randn_like(x) * math.sqrt(alpha)
    ...
    def attack(self, src,dict, models,size=(112,112)):
        ...
        for _ in range(self.iters):
            ...
            for i in train_index:
                feature1=models[i].forward(Resize_and_Padding((F.interpolate(tmp, size=size, mode='bilinear')), scale_factor))
            ...
            feature1 = model.forward(Resize_and_Padding((F.interpolate(tmp, size=size, mode='bilinear')), scale_factor))
